...
def test_invalid():
    ...
    assert log_gradient(x, y, np.array([3, 0.5, -6]).reshape((-1, 1))) is None
    assert log_gradient(x, y, np.array([1, 0, 0, 0, 0]).reshape((-1, 1))) is None
    assert log_gradient(x, y[:, 1:], np.array([1, 2, 3, 4]).reshape((-1, 1))) is None
    assert (
        log_gradient(
            x,
